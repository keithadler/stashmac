...
#   ./make-dmg.sh            -> build/Stash-for-Mac-<version>.dmg
...
DMG="build/Stash-for-Mac-$VERSION.dmg"
STAGE=$(mktemp -d)
...
Stash for Mac — how to install

1. Drag the "Stash for Mac" icon onto the "Applications" folder next to it.
...
2. Open the app: open Launchpad (the rocket in the Dock) or your Applications
   folder and click "Stash for Mac".
...
4. Click the Apple menu (top-left) > System Settings > Privacy & Security.
   Scroll all the way down. Next to "Stash for Mac was blocked", click
...
5. Done. It opens like any other app from now on, and macOS never asks again.
   Choose the folders to protect and where the backup goes, then keep the recovery
   card the app shows you somewhere safe. Without it the backup cannot be read.
...
https://github.com/keithadler/stashmac
TXT

rm -f "$DMG"
hdiutil create -volname "Stash for Mac" -srcfolder "$STAGE" -ov -format UDZO -quiet "$DMG"
